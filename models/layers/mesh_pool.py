...
class MeshPool(nn.Module):
    def __init__(self, target, multi_thread=False):
        ...
        self.__edge_priorities = None

    def __call__(self, fe, meshes, edge_priorities=None):
        return self.forward(fe, meshes, edge_priorities)

    def forward(self, fe, meshes, edge_priorities=None):
        self.__updated_fe = [[] for _ in range(len(meshes))]
        pool_threads = []
        self.__fe = fe
        self.__meshes = meshes
        self.__edge_priorities = edge_priorities
        # iterate over batch
    ...
    @staticmethod
    def __build_l2_queue(features, edges_count):
        # delete edges with smallest norm
        squared_magnitude = torch.sum(features * features, 0)
        return MeshPool.__build_priority_queue(squared_magnitude, edges_count)

    @staticmethod
    def __build_priority_queue(edge_priorities, edges_count):
        # delete edges with smallest given priority
        if edge_priorities.shape[-1] != 1:
            edge_priorities = edge_priorities.unsqueeze(-1)
        edge_ids = torch.arange(edges_count, device=edge_priorities.device, dtype=torch.float32).unsqueeze(-1)
        heap = torch.cat((edge_priorities, edge_ids), dim=-1).tolist()
        heapify(heap)
        return heap
